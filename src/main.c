  ...
  switch(seconds) {
    ...
    case 0:
      ...
      if((g_state_now[3] != g_state_prev[3]) || (DEBUG_MODE)) {
        util_animate_layer(text_layer_get_layer(g_digits[4]), GRect(MINS_UNITS_X, -50, 50, 60), GRect(MINS_UNITS_X, 53, 50, 60), LOWER_DURATION, 100);
      ...
      if((g_state_now[2] != g_state_prev[2]) || (DEBUG_MODE)) {
        util_animate_layer(text_layer_get_layer(g_digits[3]), GRect(MINS_TENS_X, -50, 50, 60), GRect(MINS_TENS_X, 53, 50, 60), LOWER_DURATION, 180);
        util_animate_layer(get_inv_layer(s_beams[2]), GRect(MINS_TENS_X + X_OFFSET, 0, BEAM_WIDTH, BEAM_HEIGHT), GRect(MINS_TENS_X + X_OFFSET, 0, BEAM_WIDTH, 0), 400, 580);
        g_state_prev[2] = g_state_now[2];
      }
      if((g_state_now[1] != g_state_prev[1]) || (DEBUG_MODE)) {
        util_animate_layer(text_layer_get_layer(g_digits[1]), GRect(HOURS_UNITS_X, -50, 50, 60), GRect(HOURS_UNITS_X, 53, 50, 60), LOWER_DURATION, 260);
        util_animate_layer(get_inv_layer(s_beams[1]), GRect(HOURS_UNITS_X + X_OFFSET, 0, BEAM_WIDTH, BEAM_HEIGHT), GRect(HOURS_UNITS_X + X_OFFSET, 0, BEAM_WIDTH, 0), 400, 660);
        g_state_prev[1] = g_state_now[1];
      }
      if((g_state_now[0] != g_state_prev[0]) || (DEBUG_MODE)) {
        util_animate_layer(text_layer_get_layer(g_digits[0]), GRect(HOUR_TENS_X, -50, 50, 60), GRect(HOUR_TENS_X, 53, 50, 60), LOWER_DURATION, 340);
        util_animate_layer(get_inv_layer(s_beams[0]), GRect(HOUR_TENS_X + X_OFFSET, 0, BEAM_WIDTH, BEAM_HEIGHT), GRect(HOUR_TENS_X + X_OFFSET, 0, BEAM_WIDTH, 0), 400, 740);
        g_state_prev[0] = g_state_now[0];
      }
       
      // Bottom surface down
      util_animate_layer(get_inv_layer(s_seconds_layer), GRect(0, 105, 144, 5), GRect(0, 105, 0, 5), 440, 560);
      break;

    // Back-up for bad animation at t=2s
    case 2:
    ...
    // 15 seconds bar
    case 15:
      util_animate_layer(get_inv_layer(s_seconds_layer), GRect(0, 105, 0, 5), GRect(0, 105, 36, 5), 800, 0);
      break;

    // 30 seconds bar
    case 30:
      util_animate_layer(get_inv_layer(s_seconds_layer), GRect(0, 105, 36, 5), GRect(0, 105, 72, 5), 800, 0);
      break;

    // 45 seconds bar
    case 45:
      util_animate_layer(get_inv_layer(s_seconds_layer), GRect(0, 105, 72, 5), GRect(0, 105, 108, 5), 800, 0);
      break;

    // Complete bar
    case 58:
      util_animate_layer(get_inv_layer(s_seconds_layer), GRect(0, 105, 108, 5), GRect(0, 105, 144, 5), 800, 1000);
      break;
    ...
    case 59:
      ...
      if((g_state_now[3] != g_state_prev[3]) || (DEBUG_MODE)) {
        util_animate_layer(get_inv_layer(s_beams[3]), GRect(MINS_UNITS_X + X_OFFSET, 0, BEAM_WIDTH, 0), GRect(MINS_UNITS_X + X_OFFSET, 0, BEAM_WIDTH, BEAM_HEIGHT), 200, 240);
        util_animate_layer(text_layer_get_layer(g_digits[4]), GRect(MINS_UNITS_X, 53, 50, 60), GRect(MINS_UNITS_X, -50, 50, 60), LOWER_DURATION, 940);
      }
       
      // Only change minutes tens if its changed
      if((g_state_now[2] != g_state_prev[2]) || (DEBUG_MODE)) {
        util_animate_layer(get_inv_layer(s_beams[2]), GRect(MINS_TENS_X + X_OFFSET, 0, BEAM_WIDTH, 0), GRect(MINS_TENS_X + X_OFFSET, 0, BEAM_WIDTH, BEAM_HEIGHT), 200, 160);
        util_animate_layer(text_layer_get_layer(g_digits[3]), GRect(MINS_TENS_X, 53, 50, 60), GRect(MINS_TENS_X, -50, 50, 60), LOWER_DURATION, 860);
      }
       
      // Only change hours units if its changed
      if((g_state_now[1] != g_state_prev[1]) || (DEBUG_MODE)) {
        util_animate_layer(get_inv_layer(s_beams[1]), GRect(HOURS_UNITS_X + X_OFFSET, 0, BEAM_WIDTH, 0), GRect(HOURS_UNITS_X + X_OFFSET, 0, BEAM_WIDTH, BEAM_HEIGHT), 200, 80);
        util_animate_layer(text_layer_get_layer(g_digits[1]), GRect(HOURS_UNITS_X, 53, 50, 60), GRect(HOURS_UNITS_X, -50, 50, 60), LOWER_DURATION, 780);
      }
       
      // Only change hours tens if its changed
      if((g_state_now[0] != g_state_prev[0]) || (DEBUG_MODE)) {
        util_animate_layer(get_inv_layer(s_beams[0]), GRect(HOUR_TENS_X + X_OFFSET, 0, BEAM_WIDTH, 0), GRect(HOUR_TENS_X + X_OFFSET, 0, BEAM_WIDTH, BEAM_HEIGHT), 200, 0);
        util_animate_layer(text_layer_get_layer(g_digits[0]), GRect(HOUR_TENS_X, 53, 50, 60), GRect(HOUR_TENS_X, -50, 50, 60), LOWER_DURATION, 700);
      }
...
static void window_load(Window *window) {
  ...
  g_day_layer = util_gen_text_layer(GRect(45,105,100,30), fg_color, GColorClear, true, RESOURCE_ID_FONT_IMAGINE_24, NULL, GTextAlignmentLeft);
...
//  if(comm_get_setting(PERSIST_KEY_DATE)) {
    layer_add_child(window_layer, text_layer_get_layer(g_day_layer));
...
static void window_unload(Window *window) {  
  ...
  text_layer_destroy(g_day_layer);
